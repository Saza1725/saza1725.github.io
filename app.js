...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadNews(){
    ...
    for(const file of files){
      ...
      } catch(e){ console.warn("News Fehler", e); }
    }
    ...
    if(allNews.length > 0 && allNews[0].time > latestNewsTimestamp){
      ...
      if(newsBell) newsBell.classList.add("new");
    }
  ...
  /* ================= OPEN FUNCTIONS ================= */
  function openMeineZeit(){ document.dispatchEvent(new CustomEvent("openSection",{detail:"meinezeitOverlay"})); }
  function openArchive(){ document.dispatchEvent(new CustomEvent("openSection",{detail:"archiveOverlay"})); }
  function openPersonal(){ document.dispatchEvent(new CustomEvent("openSection",{detail:"personalOverlay"})); }
  ...
  if(DEV_MODE){ overlay.style.display="none"; } 
    ...
    function typeWriter(){
      if(i<introText.length){ textEl.textContent += introText.charAt(i); i++; setTimeout(typeWriter,50); }
      else { button.disabled=false; button.classList.add("active"); }
    }
    ...
    button.onclick = () => { overlay.style.opacity=0; setTimeout(()=>overlay.style.display="none",800); audio.pause(); };
  }
