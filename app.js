document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.querySelectorAll("#menu a").forEach(a=>{
    a.addEventListener("click", e=>{
      ...
      if(overlay){ overlay.style.display="flex"; overlay.classList.remove("hidden"); main.style.display="none"; }
    });
  });
  ...
  if(tageszeitData){
    ...
    dailyQuoteBox.innerText = tageszeitData.quotes[seed % tageszeitData.quotes.length];
  }
  ...
  if(dailyTextsData){
    ...
    function show(type){
      personalQuoteDisplay.innerText = dailyTextsData[type][seed % dailyTextsData[type].length];
  ...
  if(foldersData){
    ...
    Object.keys(foldersData.folders).forEach(name=>{
      ...
      div.onclick=()=>{
        ...
        content.innerHTML=`<h3>${name}</h3>`+foldersData.folders[name].flat().map(q=>`<p>${q}</p>`).join("")+`<button class="closeBtn">Schließen</button>`;
  ...
  if(archiveData){
    ...
    archiveData.days.forEach(entry=>{
  ...
  if(meineZeitData){
    ...
    function renderFolder(name){
      ...
      backBtn.innerText="← Zurück"; backBtn.className="closeBtn";
  ...
  personalData = await safeFetchJSON("data/personalSlides.json");
  ...
  function renderSlides(){
    ...
    mainSlide.style.display="none"; mainSlide.classList.remove("active");
    personalData.slides.forEach((slide,i)=>{
      const div=document.createElement("div");
      div.className="personalSlide"; if(i===index) div.classList.add("active");
  ...
  async function loadNews(){
    ...
    const archive = await safeFetchJSON("data/archive.json");
    if(archive) archive.days.forEach(e=>allEntries.push({source:"Archiv", location:"Archiv", date:e.date, text:e.quote}));

    const meinezeit = await safeFetchJSON("data/meinezeit.json");
    if(meinezeit){
